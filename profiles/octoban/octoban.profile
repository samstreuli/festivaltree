 ...
 */
function octoban_form_install_configure_form_alter(&$form, $form_state) {
  // Pre-populate the site name with the server name.
  $form['site_information']['site_name']['#default_value'] = $_SERVER['SERVER_NAME'];
  $form['#submit'][] = 'octoban_form_submit';
}


function octoban_form_submit($form, &$form_state) {
  // Import database octoban file.
  $octoban_file = dirname(__FILE__) . '/octoban.sql';
  $success = import_octoban($octoban_file);
...
/**
 * Imports a database octoban file.
 *
 * @see demo_reset().
 */
function import_octoban($filename) {
  ...
  if (!file_exists($filename) || !($fp = fopen($filename, 'r'))) {
    drupal_set_message(t('Unable to open octoban file %filename.', array('%filename' => $filename)), 'error');
    return FALSE;
  }
